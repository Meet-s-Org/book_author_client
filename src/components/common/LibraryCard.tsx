...
import { FaUser, FaBook, FaEdit, FaTrash, FaInfoCircle } from "react-icons/fa";
      ...
      <div className="w-full h-full bg-white rounded-xl shadow-md hover:shadow-lg transition duration-200 hover:-translate-y-1 p-4 flex flex-col justify-between">
        <div className="card-body p-0 flex flex-col justify-between">
          <div>
            ...
            {type === "author" ? (
              <>
                ...
                {item.books?.length ? (
                  <p className="text-xs font-medium text-blue-600 underline" onClick={() => setShowBookModal(true)}>
                    {item.books.length} Book(s) <FaInfoCircle className="inline ml-1" />
                  </p>
                ) : (
                  <p className="text-xs font-medium text-blue-600">No books</p>
                )}
              </>
            ) : (
              <>
                <p className="text-xs text-gray-500 line-clamp-2">{item.description}</p>
                <p className="text-xs font-medium text-gray-600 mt-1">Published: {item.published_date}</p>
                {item.author && (
                  <p className="text-xs font-medium text-blue-600 underline" onClick={() => setShowAuthorModal(true)}>
                    Author Info <FaInfoCircle className="inline ml-1" />
                  </p>
                )}
              </>
            )}
          ...
          <div className="card-actions justify-end mt-2 gap-2">
            <button className="btn btn-xs btn-outline btn-success" onClick={() => onEdit(item)} title="Edit">
              <FaEdit />
            </button>
            <button className="btn btn-xs btn-outline btn-error" onClick={() => onDelete(item.id)} title="Delete">
              <FaTrash />
            </button>
          </div>
        </div>
